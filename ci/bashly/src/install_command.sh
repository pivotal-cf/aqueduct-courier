...
export TELEMETRY_TILE_GUID=$(smith om --lockfile="${PWD}/shepherd_envs/${args[foundation]}-metadata.json" -- curl -s --path /api/v0/deployed/products | jq -r '.[] | select(.type == "pivotal-telemetry-om").guid') || ""
...
LOADER_API_KEY=$(vault kv get -format=json /runway_concourse/tanzu-portfolio-insights/aqueduct-loader | jq -r '.data' | jq -r '.["production-loader-api-keys"]' | jq -r '.["Telemetry and Insights"]' | jq -r '.[0]')
...
LOG_FILE=/tmp/bashly/logs/${args[foundation]}.log
touch $LOG_FILE
